...
```{r ppo-clip-obj, out.width='80%', fig.show='hold', fig.cap='The clipped objective function in PPO (from the original PPO paper).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Policy-Gradients/PPO_clip.png')
```
...
Standard actor–critic methods maximize expected return. Soft Actor–Critic (SAC) augments the objective with an entropy bonus that explicitly encourages exploration and robustness while remaining off-policy and sample efficient [@haarnoja2018soft]. We first introduce a minimal implementation of SAC for discrete actions, then present full SAC with additional techniques for continuous actions.
...
To show this, consider the function $\log t$ that is concave for $t > 0$. Using [Jensen's inequality](https://en.wikipedia.org/wiki/Jensen's_inequality), we have that
\begin{equation}
\begin{split}
...
(\#eq:Bellman-Consistency-Soft-Q)
\end{equation}

**Critic Update.** For a replay sample \((s,a,r,s')\), assuming discrete actions, we can compute the target $Q$ value following the soft Bellman Consistency equation \@ref(eq:Bellman-Consistency-Soft-Q)
\begin{equation}
y \;=\;
...
\right].
(\#eq:sac-actor-grad-discrete)
\end{equation}

The following pseudocode implements a basic SAC algorithm with discrete actions.

::: {.highlightbox}
<div style="text-align:center;">**Soft Actor–Critic (Discrete Actions, Single Q + Single Target)**</div>
...
3. **Target computation (single target network).**
   ...
   - **Bellman target:**  
     ...
     \]
     *(Matches \@ref(eq:sac-target) with one target network.)*

4. **Critic update.** 
    Minimize the squared error (cf. \@ref(eq:sac-critic-loss)):  
     \[
     \psi \leftarrow \psi - \alpha_\psi \nabla_{\psi}\;\frac{1}{B}\sum\nolimits_{(s,a,r,s',d)}
     \big(Q_{\psi}(s,a)-y\big)^2.
     \]

5. **Actor update.** Minimize (cf. \@ref(eq:sac-actor-discrete)):  
     \[
     \theta \leftarrow \theta - \alpha_\theta \nabla_\theta \frac{1}{B}\sum_s
     \sum_{a} \pi_\theta(a\mid s)\,\Big(\alpha\,\log \pi_\theta(a\mid s)-Q_{\psi}(s,a)\Big).
     \]

7. **Target critic (Polyak).**  
   \[
   \bar\psi \;\leftarrow\; \tau\,\psi \;+\; (1-\tau)\,\bar\psi.
   \]
:::
...
::: {.examplebox}
::: {.example #cartpole-sac name="SAC for Cart-pole Balancing"}
...
You can find the code [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/cartpole_sac.py). Play with the temperature parameter.

```{r cart-pole-learning-curve-sac, out.width='60%', fig.show='hold', fig.cap='Learning curve (Soft Actor--Critic).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Policy-Gradients/cartpole_learning_curve_sac.png')
```

```{r cart-pole-policy-rollout-sac, out.width='60%', fig.show='hold', fig.cap='Policy rollout (Soft Actor--Critic).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Policy-Gradients/cartpole-sac-rollout.gif')
```
:::
:::
...
**Critic Update.** Maintain two critics \(Q_{\psi_1},Q_{\psi_2}\) and their target copies
...
**Tuning \(\alpha\) (Temperature).** \(\alpha\) trades off reward pursuit vs. policy entropy. A fixed \(\alpha\) is problem-dependent. SAC treats \(\alpha\) as a dual variable to enforce a target entropy \(\bar{\mathcal H}\) (often \(-\text{dim}(\mathcal A)\)):
\[
...
::: {.highlightbox}
<div style="text-align:center;">**Soft Actor–Critic (Continuous Actions, Twin Critics + Twin Targets)**</div>
...
::: {.examplebox}
::: {.example #pendulum-sac name="SAC for Inverted Pendulum"}
...
Code can be found [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/pendulum_sac.py).

```{r pendulum-learning-curve-sac, out.width='60%', fig.show='hold', fig.cap='Learning curve (Soft Actor--Critic).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Policy-Gradients/pendulum_learning_curve_sac.png')
```

```{r pendulum-policy-rollout-sac, out.width='40%', fig.show='hold', fig.cap='Policy rollout (Soft Actor--Critic).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Policy-Gradients/pendulum_rollout_sac-1.gif')
knitr::include_graphics('images/Policy-Gradients/pendulum_rollout_sac-2.gif')
```
...
In continuous-control tasks, sampling or integrating over actions inside policy gradients is costly and noisy. The Deterministic Policy Gradient (DPG) framework [@silver2014deterministic] replaces the stochastic policy \(\pi_\theta(a\mid s)\) with a *deterministic* actor
\[
...
The on-policy objective \@ref(eq:DPG-obj-on) is the usual RL objective in policy gradient methods, as $Q^{\mu_\theta}(s, \mu_\theta(s)) = V^{\mu_\theta}(s)$ by definition. 

A key result in Deterministic Policy Gradient is that under mild conditions, optimizating the surrogate off-policy objective \@ref(eq:DPG-obj-off) is the same as optimizating the original on-policy objective.
...
In particular, the off-policy DPG \@ref(eq:DPG-grad-off) can be estimated from replay sampled under \(\beta\) without action-importance ratios; only the state weighting changes.

The following result states that the on-policy and off-policy objectives share the same stationary points.

::: {.theorembox}
::: {.theorem #dpg-common-foc name="Common First-Order Optima"}
Let
\begin{equation}
g(s;\theta)\;:=\;\nabla_\theta \mu_\theta(s)\,\nabla_a Q^{\mu_\theta}(s,a)\big|_{a=\mu_\theta(s)}\in\mathbb{R}^{d},
(\#eq:per-state-gradient)
\end{equation}
...
**From DPG to DDPG (Deep DPG).** DDPG [@lillicrap2015continuous] implements DPG with deep networks + standard stabilizers:
...
::: {.highlightbox}
<div style="text-align:center;">**Deep Deterministic Policy Gradient (DDPG)**</div>
...
::: {.examplebox}
::: {.example #pendulum-ddpg name="DDPG for Inverted Pendulum"}
...
Code can be found [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/pendulum_ddpg.py).

```{r pendulum-learning-curve-ddpg, out.width='60%', fig.show='hold', fig.cap='Learning curve (DDPG).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Policy-Gradients/pendulum_learning_curve_ddpg.png')
```

```{r pendulum-policy-rollout-ddpg, out.width='30%', fig.show='hold', fig.cap='Policy rollout (DDPG).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Policy-Gradients/pendulum_rollout_ddpg-1.gif')
knitr::include_graphics('images/Policy-Gradients/pendulum_rollout_ddpg-2.gif')
knitr::include_graphics('images/Policy-Gradients/pendulum_rollout_ddpg-3.gif')
```
...
<!-- Model-based policy optimization (MBPO) [@janner2019trust] sits between pure model-free methods (high variance, data hungry) and “plan-only” model-based control (sensitive to model bias). The key idea is **to learn a dynamics model and then use only *short* rollouts from that model to create extra training data** for a strong off-policy learner (usually SAC). Short horizons curb compounding model error; a large model-generated replay reduces variance and boosts sample efficiency.

## High-level idea

Consider an MDP with unknown dynamics \(s_{t+1} \sim p(s_{t+1}\mid s_t,a_t)\). MBPO learns an ensemble \(\{\hat f_{\psi_k}\}_{k=1}^K\) that predicts the next state (often the *delta-state* \(\Delta s\)). Rather than planning far ahead inside the learned model, MBPO:

1. Collects **real** transitions \(\mathcal{D}_{\text{env}}\) by interacting with the environment.
2. **Fits** the dynamics ensemble on \(\mathcal{D}_{\text{env}}\).
3. Periodically **generates short rollouts** (e.g., horizon \(H=1\ldots5\)) starting from **real** states by simulating with a *random* member of the ensemble, producing **model** transitions \(\mathcal{D}_{\text{model}}\).
4. **Trains** an off-policy actor-critic (e.g., SAC) on a **mixture** of \(\mathcal{D}_{\text{env}}\) and \(\mathcal{D}_{\text{model}}\), typically with a high fraction of model data but *short* \(H\) to limit bias.
...
## Algorithm: MBPO (short-model-rollout variant)

::: {.highlightbox}
**Inputs:** environment \(\mathcal{E}\), policy \(\pi_\theta(a\mid s)\), critics \(Q_{\phi_1},Q_{\phi_2}\) with targets, temperature \(\alpha\) (auto-tuned), dynamics ensemble \(\{\hat f_{\psi_k}\}_{k=1}^K\), real buffer \(\mathcal{D}_{\text{env}}\), model buffer \(\mathcal{D}_{\text{model}}\). Rollout horizon \(H\), model ratio \(p_{\text{model}}\in[0,1]\), update counts \(G_{\text{dyn}}, G_{\text{rl}}\).
...
2. **Fit dynamics.** For \(G_{\text{dyn}}\) steps:
   - Sample minibatch \(B\subset\mathcal{D}_{\text{env}}\).
   - Update each \(\psi_k\) to predict \(\Delta s = s'-s\) (and optionally \(r\)) by minimizing NLL/MSE.

3. **Short model rollouts (data generation).**
   - Sample seed states \(S_0\) from recent \(\mathcal{D}_{\text{env}}\).
   - For each \(s\in S_0\): for \(h=1\ldots H\):
...
4. **Off-policy RL updates.** For \(G_{\text{rl}}\) steps:
...
5. **Repeat** steps 1–4 until budget is exhausted. Tune \(H\) and \(p_{\text{model}}\) to trade off bias/variance.
:::
...
## Minimal MBPO on `Pendulum-v1` (Gym/Gymnasium, PyTorch)

Below is a compact, instructional script. It uses:
- A **dynamics ensemble** that predicts \(\Delta s\) (we compute the true reward analytically for Pendulum).
- A **SAC** learner (Gaussian policy with tanh-squash) trained on a **mixture** of real and model transitions.
- **Short rollouts** (default \(H=1\)).

```python
# MBPO on Pendulum-v1 (PyTorch)
# --------------------------------
# pip install torch gymnasium  (or: pip install torch gym)
...
# Pendulum reward (from Gym source)
...
class ReplayBuffer:
    def __init__(self, obs_dim, act_dim, size, device):
        ...
        self.next_buf = np.zeros((size, obs_dim), dtype=np.float32)
        self.done_buf = np.zeros((size, 1), dtype=np.float32)
        self.max_size, self.ptr, self.size = size, 0, 0
        self.device = device

    def push(self, s, a, r, s2, d):
        self.obs_buf[self.ptr]  = s
    ...
    def sample(self, batch_size):
        ...
        to_t = lambda x: torch.as_tensor(x[idxs], dtype=torch.float32, device=self.device)
        return to_t(self.obs_buf), to_t(self.act_buf), to_t(self.rew_buf), to_t(self.next_buf), to_t(self.done_buf)
...
class ActorGaussian(nn.Module):
    ...
    def forward(self, obs):
        ...
        act_dim = mu_logstd.shape[-1] // 2
        mu, log_std = mu_logstd[..., :act_dim], mu_logstd[..., act_dim:]
        log_std = torch.clamp(log_std, -20, 2)
        std = torch.exp(log_std)
        dist = torch.distributions.Normal(mu, std)
        eps = dist.rsample()                          # reparameterized
        squashed = torch.tanh(eps)
        action = self.act_scale.to(obs.device) * squashed + self.act_bias.to(obs.device)
        # Tanh correction (sum over action dims)
        logp = dist.log_prob(eps) - torch.log(1 - squashed.pow(2) + 1e-6)
    ...
    def act(self, obs, deterministic=False):
        ...
        if deterministic:
            ...
            squashed = torch.tanh(eps)
        action = self.act_scale * squashed + self.act_bias
...
class Ensemble:
    def __init__(self, n_models, obs_dim, act_dim, device):
        ...
        self.device  = device
        self.n = n_models
    def train_step(self, batch_s, batch_a, batch_s2):
        losses = []
        for m,opt in zip(self.members, self.optims):
            opt.zero_grad()
            pred = m(batch_s, batch_a)
            target = batch_s2 - batch_s
    ...
    def step(self, s, a, sigma=0.0):
        # sample a random member per call
...
class MBPOConfig:
    ...
    sac_updates_per_step: float = 1.0   # RL updates per env step
    model_train_iters: int = 200        # per model-updating phase
    rollout_horizon: int = 1            # short horizon is key
    model_ratio: float = 0.9            # fraction from model buffer
    gamma: float = 0.99
    tau: float = 0.005
    lr_actor: float = 3e-4
...
def train_mbpo(config=MBPOConfig()):
    device = torch.device(config.device)
    env = make_env(SEED)
    obs, _info = env.reset() if GYMN else env.reset()
    ...
    # Replay buffers
    rb_real  = ReplayBuffer(obs_dim, act_dim, config.real_buffer_size, device)
    rb_model = ReplayBuffer(obs_dim, act_dim, config.model_buffer_size, device)

    # SAC nets
    actor = ActorGaussian(obs_dim, act_dim, act_low, act_high).to(device)
    critic = CriticTwin(obs_dim, act_dim).to(device)
    critic_targ = CriticTwin(obs_dim, act_dim).to(device)
    critic_targ.load_state_dict(critic.state_dict())

    pi_optim = torch.optim.Adam(actor.parameters(), lr=config.lr_actor)
    q_optim  = torch.optim.Adam(critic.parameters(), lr=config.lr_critic)

    # Entropy temperature (auto)
    target_entropy = -float(act_dim)
    log_alpha = torch.tensor(0.0, requires_grad=True, device=device)
    alpha_optim = torch.optim.Adam([log_alpha], lr=config.lr_alpha)

    # Dynamics ensemble
    ensemble = Ensemble(config.ensemble_size, obs_dim, act_dim, device)

    ep_ret, ep_len = 0.0, 0
    returns = []
    s = obs
    for t in range(1, config.total_env_steps + 1):
        # ---- Select action ----
        if t < config.start_random_steps:
        ...
        rb_real.push(s, a, r, s2, float(done))
        ep_ret += r; ep_len += 1
        s = s2
        ...
        if t >= config.update_after and t % config.update_every == 0:
            ...
            for _ in range(config.model_train_iters):
                if rb_real.size < config.real_batch_size: break
                bs = config.real_batch_size
                s_b, a_b, _r_b, s2_b, _d_b = rb_real.sample(bs)
                ensemble.train_step(s_b, a_b, s2_b)

            # 2) Generate short model rollouts seeded from real states
            seeds = 1000  # number of seed states per generation burst
            if rb_real.size > seeds:
                # recent indices for more on-distribution seeds
                idxs = np.random.randint(max(0, rb_real.size - 5000), rb_real.size, size=seeds)
                seed_states = torch.as_tensor(rb_real.obs_buf[idxs], dtype=torch.float32, device=device)
                s_model = seed_states.clone()
                for _h in range(config.rollout_horizon):
                    with torch.no_grad():
                        a_model, _logp = actor(s_model)
                        s2_model = ensemble.step(s_model, a_model, sigma=0.0)  # keep deterministic for stability
                        # compute true Pendulum reward analytically
                        r_list = []
                        for i in range(s_model.shape[0]):
                            r_i = pendulum_reward(s_model[i].cpu().numpy(), a_model[i].cpu().numpy())
                            r_list.append([r_i])
                        r_model = torch.tensor(r_list, dtype=torch.float32, device=device)
                        d_model = torch.zeros_like(r_model)  # no terminal signals from short model rollouts
                    # push into model buffer
                    for i in range(s_model.shape[0]):
                        rb_model.push(s_model[i].cpu().numpy(),
                                      a_model[i].cpu().numpy(),
                                      r_model[i].cpu().numpy(),
                                      s2_model[i].cpu().numpy(),
                                      d_model[i].cpu().numpy())
                    s_model = s2_model

            # 3) Off-policy SAC updates
            total_updates = int(config.sac_updates_per_step * config.update_every)
            for _ in range(total_updates):
                if rb_real.size < config.real_batch_size: continue
                # sample mixture
                bm = max(1, int(config.model_ratio * config.model_batch_size))
                br = max(1, config.model_batch_size - bm)
                # If model buffer is too small, fall back to real
                if rb_model.size < bm:
                    bm = 0; br = config.model_batch_size
                if br > 0:
                    s_r, a_r, r_r, s2_r, d_r = rb_real.sample(br)
                if bm > 0:
                    s_m, a_m, r_m, s2_m, d_m = rb_model.sample(bm)

                if bm > 0 and br > 0:
                    s_b  = torch.cat([s_r,  s_m],  dim=0)
                    a_b  = torch.cat([a_r,  a_m],  dim=0)
                    r_b  = torch.cat([r_r,  r_m],  dim=0)
                    s2_b = torch.cat([s2_r, s2_m], dim=0)
                    d_b  = torch.cat([d_r,  d_m],  dim=0)
                elif bm > 0:
                    s_b, a_b, r_b, s2_b, d_b = s_m, a_m, r_m, s2_m, d_m
                else:
                ...
                with torch.no_grad():
                    ...
                    q_targ = torch.min(q1_t, q2_t) - torch.exp(log_alpha) * logp2
                    y = r_b + (1.0 - d_b) * config.gamma * q_targ
                ...
                pi_loss = (torch.exp(log_alpha) * logp - q_pi).mean()
                ...
                # Targets
                soft_update(critic, critic_targ, config.tau)

        if t % 2000 == 0 and len(returns) > 5:
            print(f"[t={t}] recent avg return: {np.mean(returns[-5:]):.1f}, real_buf={rb_real.size}, model_buf={rb_model.size}")

    env.close()
    print(f"Training done. Episodes logged: {len(returns)}. Last 10 avg return: {np.mean(returns[-10:]) if len(returns)>=10 else np.mean(returns):.1f}")
    return returns

if __name__ == "__main__":
    cfg = MBPOConfig(
        total_env_steps = 40_000,
        rollout_horizon = 1,      # try 1-3; larger increases model bias
        model_ratio     = 0.9,    # more model data after warmup
        device          = "cpu"
    )
    train_mbpo(cfg)
``` -->
