...
#### Multi-Step TD Learning {#PE-MultiStepTD}
...
#### Eligibility Traces and TD(\(\lambda\)) {#TDlambda}
...
### Convergence Proof of TD Learning {#value-rl-convergence-td}
...
(\#eq:StationaryDistribution-Tabular)
\end{equation}
If we denote
\begin{equation}
P^\pi(s' \mid s) = \sum_{a \in \mathcal{A}} \pi(a \mid s) P(s' \mid s,a),
(\#eq:TabularStateOnlyTransition)
\end{equation}
as the $\pi$-induced state-only transition dynamics, then condition \@ref(eq:StationaryDistribution-Tabular) is equivalent to 
\begin{equation}
\mu^\pi(s') = \sum_{s \in \mathcal{S}} \mu^\pi(s) P^\pi (s' \mid s), \quad \forall s' \in \mathcal{S}.
(\#eq:StationaryDistribution-Tabular-State-Only)
\end{equation}

See \@ref(eq:StationaryDistribution-definition) for a generalization to continuous MDP. 
...
Let $\mu^\pi$ be the stationary distribution in \@ref(eq:StationaryDistribution-Tabular-State-Only), \(D=\mathrm{diag}(\mu^\pi)\), and \(w:=V-V^\pi\). Then
\[
...
(\#eq:InnerProductInequality)
\end{equation}

Next, we prove \(\langle w, D\widetilde P w\rangle\le \|w\|_D^2\).

- First, we show \(\|\widetilde P w\|_D\le \|w\|_D\). For any state $s \in \mathcal{S}$, from \@ref(eq:def-tilde-P), we have
$$
(\widetilde P w)(s) = \sum_{s'} P^\pi (s' \mid s) w(s'),
$$
where $P^\pi(s' \mid s)$ is the $\pi$-induced state-only transition in \@ref(eq:TabularStateOnlyTransition). Since $P^\pi(\cdot \mid s)$ is a probability distribution, and $x \mapsto x^2$ is convex, we have 
$$
...
\end{split}
\end{equation}
where the second-from-last equality holds because $\mu^\pi$ is the stationary distribution and satisfies \@ref(eq:StationaryDistribution-Tabular-State-Only).
...
Plugging this back to \@ref(eq:InnerProductInequality), we obtain
$$
...
```{r Mountain-car-plot, out.width='50%', fig.show='hold', fig.cap='Mountain Car from Gym', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mountain_car.gif')
```
...
You can find code for the naive semi-gradient SARSA algorithm [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/mountaincar_sarsa.py).

```{r Mountain-car-sarsa-episode-return, out.width='60%', fig.show='hold', fig.cap='Average return w.r.t. episode (Semi-Gradient SARSA)', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mountain-car_sarsa_eps3000_seed50_returns.png')
```

```{r Mountain-car-sarsa-rollout, out.width='60%', fig.show='hold', fig.cap='Example rollout (Semi-Gradient SARSA)', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mountain-car_sarsa_eps3000_seed50_greedy-episode-0.mp4')
```
...
You can find code for the semi-gradient SARSA with experience replay algorithm [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/mountaincar_sarsa_er.py).

```{r Mountain-car-sarsa-er-episode-return, out.width='60%', fig.show='hold', fig.cap='Average return w.r.t. episode (Semi-Gradient SARSA + Experience Replay)', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mountain-car_sarsa+er_eps3000_seed50_returns.png')
```

```{r Mountain-car-sarsa-er-rollout, out.width='60%', fig.show='hold', fig.cap='Example rollout (Semi-Gradient SARSA + Experience Replay)', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mountain-car_sarsa+er_eps3000_seed50_greedy-episode-0.mp4')
```
:::
:::
...
To illustrate why, we first present off-policy semi-gradient TD(0) for policy evaluation and use Baird's counterexample [@baird1995residual] to highlight the deadly triad: bootstrapping + function approximation + off-policy sampling can cause divergence. 

We then turn to the Deep Q-Network (DQN) [@mnih2015human], which stabilizes Q-learning with two key mechanisms—experience replay and a target network—leading to landmark Atari results. Finally, we connect DQN to fitted Q-iteration (FQI) [@riedmiller2005neural], a batch method with theoretical guarantees, to clarify why these stabilizations work [@fan2020theoretical].
...
(\#eq:FAOffPolicySemiGradientTD0)
\end{equation}
where $\rho_t$ is the likelihood ratio as in \@ref(eq:IS-LikelihoodRatio):
$$
\rho_t = \frac{\pi(a_t \mid s_t)}{b(a_t \mid s_t)}.
$$
This off-policy semi-gradient TD(0) update \@ref(eq:FAOffPolicySemiGradientTD0) looks perfectly reasonable. However, the following Baird's counterexample illustrates the instability of the algorithm.


**Baird's Counterexample.** Consider an MDP with 7 states containing 6 upper states and 1 lower state, as shown in Fig. \@ref(fig:Baird-example) (the figure comes from [@sutton1998reinforcement]). There are two actions, one called "solid" and the other called "dashed". If the agent picks "solid" at any state, then the system transitons to the lower state with probability 1. If the agent picks "dashed" at any state, then the system transitions to any one of the upper states with equal probability. All rewards are zero, and the discount factor is $\gamma=0.99$.
...
```{r Baird-example, out.width='70%', fig.show='hold', fig.cap='Baird Counterexample', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/Baird.png')
```
...
This [Python script](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/BairdExample.py) implements the off-policy semi-gradient TD(0) algorithm with importance sampling for policy evaluation. 
...
```{r Baird-example-value, out.width='60%', fig.show='hold', fig.cap='Baird Counterexample: divergence', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/Baird-w.png')
```
...
**Why?** Recall that, using linear approximation, the on-policy Semi-Gradient TD(0) algorithm guarantees convergence to the unique fixed point of the projected Bellman equation (PBE) \@ref(eq:PBEFixedPoint), restated here
$$
...
**Naive (semi-gradient) Q-Learning with Function Approximation.** The goal is to learn \(Q^\star\) and act \(\varepsilon\)-greedily w.r.t. \(Q(\cdot,\cdot; \theta)\). The update uses a bootstrapped optimality target built from the current network.
\begin{equation}
\begin{split}
...
Although the naive Q-leanring with function approximation can be unstable, DQN has achieved great success and is also very efficient [@mnih2015human].
...
The FQI algorithm samples trajectories from a fixed distribution $\mu$ and optimizes the parameter using targets generated from those trajectories. Under reasonable coverage and approximation assumptions, it converges and admits finite-sample error bounds [@antos2007fitted], [@munos2008finite].
...
::: {.examplebox}
::: {.example #MountainCarQLearningFA name="DQN for Mountain Car"}
Consider again the Mountain car problem from Example \@ref(exm:MountainCarSemiGradientSARSA). 

**Naive Q-Learning with Function Approximation.** As shown in Fig. \@ref(fig:Mountain-car-q-episode-return) and Fig. \@ref(fig:Mountain-car-q-rollout), naive Q-leanring with function approximation fails to learn a good policy.


```{r Mountain-car-q-episode-return, out.width='60%', fig.show='hold', fig.cap='Average return w.r.t. episode (Naive Q Learning)', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mountain-car_qlearn_naive_eps2500_seed123_returns.png')
```

```{r Mountain-car-q-rollout, out.width='60%', fig.show='hold', fig.cap='Example rollout (Naive Q Learning)', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mountain-car_qlearn_naive_eps2500_seed123_greedy-episode-0.mp4')
```



**DQN with Experience Replay and Target Network.** Adding ER and TN to Q-learning leads to steady learning (Fig. \@ref(fig:Mountain-car-dqn-episode-return)) and a successful final policy (Fig. \@ref(fig:Mountain-car-dqn-rollout)).

You can find code for these experiments [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/mountaincar_q_learning.py).

```{r Mountain-car-dqn-episode-return, out.width='60%', fig.show='hold', fig.cap='Average return w.r.t. episode (DQN)', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mountain-car_dqn_eps2500_seed123_returns.png')
```

```{r Mountain-car-dqn-rollout, out.width='60%', fig.show='hold', fig.cap='Example rollout (DQN)', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mountain-car_dqn_eps2500_seed123_greedy-episode-0.mp4')
```
:::
:::
