...
1. **Monte Carlo (MC), first-visit** — episodic, sample-average updates.
...
To ensure convergence in the tabular setting and to make the comparison fair (since MC uses sample averages), the TD-family methods use *per-state diminishing step sizes* of the form
\begin{equation}
...
Fig. \@ref(fig:policy-evaluation-random-walk) shows the MSE versus episodes for MC, TD(0), 3-step TD, and TD(\(\lambda\)) under the diminishing step-size schedule \@ref(eq:per-state-decay).
...
```{r policy-evaluation-random-walk, out.width='90%', fig.show='hold', fig.cap='Policy Evaluation, MC versus TD Family', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Value-RL/mc_td_comparison.png')
```
