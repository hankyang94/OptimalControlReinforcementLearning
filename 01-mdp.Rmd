...
(\#eq:gamma-contraction-Bellman-operator)
\end{equation}
...
With the Bellman operator defined, we observe that the value function of $\pi$, denoted $V^{\pi}$ in \@ref(eq:BellmanConsistency-InfiniteHorizon-State-Value), is a **fixed point** of $T^{\pi}$. That is to say $V^{\pi}$ satisfies:
$$
T^{\pi} V^{\pi} = V^{\pi}.
$$
...
You can play with the code [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/pendulum_policy_evaluation.py).
...
d. $\Vert V^{\pi_{k+1}} - V^\star \Vert_{\infty} \leq \gamma \Vert V^{\pi_k} -  V^\star \Vert_{\infty}$, for any $k$ (i.e., contraction).
:::
:::
::: {.proofbox}
::: {.proof}
...
```{r mdp-pendulum-PI-value, out.width='80%', fig.show='hold', fig.cap='Optimal Value Function after Policy Iteration', fig.align='center', echo=FALSE}
knitr::include_graphics('images/MDP/pendulum_PI_value.png')
```

```{r mdp-pendulum-PI-policy, out.width='80%', fig.show='hold', fig.cap='Optimal Policy after Policy Iteration', fig.align='center', echo=FALSE}
knitr::include_graphics('images/MDP/pendulum_PI_policy.png')
```
...
You can play with the code [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/pendulum_policy_iteration.py).

```{r mdp-pendulum-PI-rollout-trajectory, out.width='90%', fig.show='hold', fig.cap='Optimal Trajectory of Pendulum Swing-Up', fig.align='center', echo=FALSE}
knitr::include_graphics('images/MDP/pendulum_optimal_trajectory.png')
```

```{r mdp-pendulum-PI-rollout-trajectory-value, out.width='90%', fig.show='hold', fig.cap='Optimal Trajectory of Pendulum Swing-Up Overlayed with Optimal Value Function', fig.align='center', echo=FALSE}
knitr::include_graphics('images/MDP/pendulum_optimal_trajectory_value.png')
```
...
Policy iteration—as the name suggests—iterates on *policies*: it alternates between
(1) *policy evaluation* (computing \(V^{\pi}\) for the current policy \(\pi\)) and
(2) *policy improvement* (making \(\pi\) greedy w.r.t. \(V^{\pi}\)).
...
**Initialization.** Choose any \(V_0:\mathcal{S}\to\mathbb{R}\) (e.g., \(V_0 \equiv 0\)).  
...
\quad \forall s\in\mathcal{S}.
\]
**Stopping rule.** Stop when \(\lVert V_{k+1}-V_k\rVert_\infty \le \varepsilon \) (or any chosen tolerance).
...
::: {.theorembox}
::: {.theorem #ValueIterationConvergence name="Convergence of Value Iteration"}
Let \(T^\star \) be the Bellman optimality operator,
\[
(T^\star V)(s) := \max_{a}\Big[ R(s,a) + \gamma \sum_{s'} P(s'\mid s,a)\, V(s') \Big].
\]
...
The following code performs value iteration for the inverted pendulum problem.
...
Try it for yourself [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/pendulum_value_iteration.py)!
