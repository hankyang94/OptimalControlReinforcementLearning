...
> Here we have used $(s_t, a_t)$ to represent the state and control. This departs from the notation \@ref(eq:planning-discrete-dynamics) used in the beginning. The reason for doing so will become clear soon.

**Trajectory Optimization.** At time step $t$, let the system's state be $s_t$ as given (e.g., measured from sensor data). MPC solves the following open-loop TO problem, adapted from \@ref(eq:to-general):
\begin{equation}
\begin{split}
\min_{\{x_k,u_k\}} \quad &
...
(\#eq:to-general-mpc)
\end{equation}
Notice that in \@ref(eq:to-general-mpc):
...
(\#eq:mpc-solution-t)
\end{equation}
be an optimal solution of the TO problem \@ref(eq:to-general-mpc) (e.g., obtained from IPOPT). Instead of executing the entire sequence of optimal controls $(u_0^\star, u_1^\star, \dots,  u_{N-1}^\star)$, MPC will only execute the first element. Formally, the actual control action applied to the system, denoted as $a_t$, is 
\begin{equation}
a_t = u_0^\star.
(\#eq:mpc-receding)
\end{equation}
Applying $a_t = u_0^\star$ to the real dynamics \@ref(eq:mpc-dynamics), the system will step into a new state $s_{t+1}$ at time $t+1$:
$$
s_{t+1} = f(s_t, u_0^\star, w_t).
$$
Then, MPC solves a new TO problem that is exactly the same as the problem \@ref(eq:to-general-mpc) at time $t$, except that the initial state is changed to $s_{t+1}$:
\begin{equation}
\begin{split}
\min_{\{x_k,u_k\}} \quad &
\Phi(x_N) + \sum_{k=0}^{N-1} \ell_k(x_k,u_k) \\[2mm]
...
& g_k(x_k,u_k) \le 0,\quad h_k(x_k,u_k)=0 \quad \text{(path/terminal constraints).}
\end{split}
(\#eq:to-general-mpc-t-1)
\end{equation}
Problems \@ref(eq:to-general-mpc) and \@ref(eq:to-general-mpc-t-1) are called *parametric optimization problems*, in the sense that the form of the optimization problem remains the same, but the value of the parameter $s_t$ that defines the equality constraint has changed.  

For this reason, we should restate the solution in \@ref(eq:mpc-solution-t) as
\begin{equation}
(x_0^\star(s_t), u_0^\star(s_t), x_1^\star(s_t), u_1^\star(s_t), \dots, x_{N-1}^\star(s_t), u_{N-1}^\star(s_t), x_N^\star(s_t))
(\#eq:mpc-solution-t-implicit)
\end{equation}
because they are all implicit functions of the parameter $s_t$.

Similarly, the solution to the TO problem \@ref(eq:to-general-mpc-t-1) at time $t+1$ should be denoted as 
\begin{equation}
...
u_k \in \mathbb{R}
\]
where \(p_k\) is position, \(v_k\) is velocity, and \(u_k\) is the commanded acceleration. With sampling time \(\Delta t\), the discrete-time, *nominal* dynamics are
\[
x_{k+1} = A x_k + B u_k,
\quad
...
**Control Goal.** We consider *regulation to a fixed target* \(x_{\text{goal}}\in\mathbb{R}^2\), typically the origin:
\[
x_{\text{goal}}=\begin{bmatrix}0\\0\end{bmatrix}.
\]
(Tracking a moving reference is immediate by updating \(x_{\text{goal}}\) online.)

**Finite-Horizon Trajectory Optimization (QP).** At each control time \(t\), with measured state \(s_t\), MPC solves a length-\(N\) open-loop trajectory optimization (TO) problem with decision variables \(\{x_k,u_k\}_{k=0}^{N}\) (with \(u_N\) unused):
\[
\begin{aligned}
...
The problem in \@ref(eq:di-mpc-to) is a convex QP (quadratic cost; linear dynamics and box constraints). We will write a Python code that builds this QP once with CVXPY and then only updates the Parameter for the measured state \(s_t\) and (optionally) \(x_{\text{goal}}\) at each MPC step.

**Receding Horizon (Implicit Feedback).** Let \(\{x_k^\star,u_k^\star\}\) be the optimizer of \@ref(eq:di-mpc-to). MPC applies only the first input
\[
a_t = u_0^\star,
\]
...
- **Warm-start.** Shift the previous optimal sequence \(u^\star\) left by one step and hold the last value, then roll out the dynamics from the new \(s_t\) to seed a feasible \(x\)-trajectory guess. These are passed to the solver via `warm_start=True`.
...
You can play with the code [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/mpc_double_integrator.py).

```{r mpc-double-integrator-small-noise, out.width='80%', fig.show='hold', fig.cap='MPC for double integrator (small disturbance).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Model-based-optimization/mpc_double_integrator-1.png')
```


```{r mpc-double-integrator-large-noise, out.width='80%', fig.show='hold', fig.cap='MPC for double integrator (large disturbance).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Model-based-optimization/mpc_double_integrator-2.png')
```
:::
:::
...
\end{aligned}
\]
Here, \(\ell_t\) is the stage (running) cost and \(\Phi\) is the terminal cost. A *dynamically feasible trajectory* \(\{x_t,u_t\}_{t=0}^{N-1}\) satisfies the dynamics constraints for all \(t\).
...
::: {.examplebox}
::: {.example #mppi-pendulum name="MPPI for Pendulum Swing Up"}
...
```{r pendulum-mppi-success, out.width='50%', fig.show='hold', fig.cap='Pendulum swing-up using MPPI (large exploration).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Model-based-optimization/mppi_pendulum-success.gif')
```
...
You can play with the code [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/mppi-pendulum.py).

```{r pendulum-mppi-failure, out.width='50%', fig.show='hold', fig.cap='Pendulum swing-up using MPPI (small exploration).', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Model-based-optimization/mppi_pendulum-failure.gif')
```
...
### Information-Theoretic Derivation of MPPI {#sec:mppi-it}
...
::: {.exercisebox}
::: {.exercise}
Formulate the Lagrangian for problem \@ref(eq:free-energy) and find its KKT optimality conditions.
:::
:::
...
(\#eq:free-energy-identity)
\]


**From Gibbs–Boltzmann Optimum to MPPI Updates.** Let a trajectory be \(\tau=(x_0,u_0,\dots,u_{H-1},x_H)\) with dynamics \(x_{t+1}=f_t(x_t,u_t)\).
...
\]
Because of nonlinear dynamics and the exponential tilt by \(S\), **\(q^\star\) is generally not Gaussian**, even if \(p\) is.

MPPI *approximates* \(q^\star\) by maintaining a *Gaussian over the control sequence* with the *same covariance* \(\Sigma\) and a tunable mean \(\mathbf U=(u_0,\dots,u_{H-1})\):
\[
\pi_{\mathbf U}(\mathbf v)\;=\;\mathcal N(\mathbf v;\mathbf U,\Sigma).
\]
MPPI projects \(q^\star\) onto this family by minimizing \(\mathrm{KL}\big(\pi_{\mathbf U}\,\|\,q^\star\big)\) while *holding \(\Sigma\) fixed*. For Gaussians with fixed covariance, this yields **moment matching of the mean**:
\[
...
In Section \@ref(mppi), we examined the effectiveness of one particular sampling-based planning method—MPPI—for solving trajectory-planning problems, noting that it is often less susceptible to local minima than nonlinear-programming-based approaches. More broadly, sampling-based motion planning encompasses a wide family of algorithms, extensive enough to merit an entire semester for a comprehensive treatment. For readers interested in exploring this area further, the textbooks [@lavalle2006planning] and [@choset2005principles] provide excellent overviews.

In this section, we introduce one of the most widely used sampling-based algorithms: the rapidly exploring random tree (RRT). 
We will begin with the case of *kinematic* motion planning, in which the system or robot is modeled as a point mass without dynamics, and then extend the discussion to *kinodynamic* motion planning, where the system's dynamics must be taken into account.


### Kinematic Motion Planning

**Problem Setup.** We consider a robot with configuration space \(\mathcal{C} \subset \mathbb{R}^d \). Obstacles occupy \(\mathcal{C}_\mathrm{obs}\), and the *free space* is
\[
\mathcal{C}_\mathrm{free} = \mathcal{C} \setminus \mathcal{C}_\mathrm{obs}.
\]
Given a start configuration \(q_\mathrm{start} \in \mathcal{C}_\mathrm{free}\) and a *goal region* \(\mathcal{C}_\mathrm{goal} \subset \mathcal{C}_\mathrm{free}\), the objective is to find a *collision-free path*
\[
...
\]
optionally with *goal bias*: introduce \(B \sim \mathrm{Bernoulli}(p_{\mathrm{goal}})\) and set
\[
q_{\mathrm{rand}} =
\begin{cases}
...
**Metric (Distance Function).** We need a distance \(d:\mathcal{C}\times\mathcal{C}\to \mathbb{R}_{\ge 0}\) to compare configurations. Common choices:
\[
...
**Steering / Step Map.** Move from \(q_{\mathrm{near}}\) *toward* \(q_{\mathrm{rand}}\) by at most a step size \(\eta>0\). In Euclidean \(\mathcal{C}\),
\[
...
**Collision Checking (Local Planner Validity).**
...
**Stopping / Success Criteria.** Let the goal be a set \(\mathcal{C}_{\mathrm{goal}}\) (e.g., a ball of radius \(\varepsilon\) around a target pose). Declare success when
\[
...
1. **Goal-biased sampling.** Draw \(B\sim\mathrm{Bernoulli}(p_{\mathrm{goal}})\) and set
   \[
   q_{\mathrm{rand}} \sim
   \begin{cases}
...
3. **Steering (bounded step).** Let \(\Delta := q_{\mathrm{rand}}-q_{\mathrm{near}}\) in a local chart of \(\mathcal{C}\) (with appropriate handling of angles/manifold coordinates). Define
   \[
...
6. **Success test.** If \(q_{\mathrm{new}}\in \mathcal{C}_{\mathrm{goal}}\), **terminate** and return the path formed by the unique tree route from \(q_{\mathrm{start}}\) to \(q_{\mathrm{new}}\).
...
**Cost functional.** Let \(c(\cdot)\) denote path cost (e.g., length or any additive, nonnegative, Lipschitz continuous cost). For an edge between two configurations \(q\) and \(q'\), write \(c(q\to q')\) and define
   \[
   c(q') \;=\; \min_{(q \to q') \in E(T)} \big\{c(q) + c(q\to q')\big\},
   \]
   where \(E(T)\) is the set of tree edges.

**Near set.** Rather than connecting to only the nearest node, examine a **ball of neighbors** around the new sample \(q_{\mathrm{new}}\) whose radius shrinks with \(n\):
   \[
...
2. **Nearest and steer.** Choose
   ...
   \]
   with \(\alpha=\min\{1,\eta/\|q_{\mathrm{rand}}-q_{\mathrm{near}}\|\}\). If the segment \([q_{\mathrm{near}},q_{\mathrm{new}}]\) is not collision-free, **continue**.
...
4. **Choose parent by cost.** Set provisional parent \(\bar{q}\gets q_{\mathrm{near}}\) with provisional cost
   \[
...
5. **Insert.** Add \(q_{\mathrm{new}}\) to \(V(T)\), add edge \((\bar{q}\rightarrow q_{\mathrm{new}})\) to \(E(T)\), and set \(c(q_{\mathrm{new}})\gets \bar{c}\).

6. **Rewire.** For each \(q\in\mathcal{N}_n(q_{\mathrm{new}})\) such that segment \([q_{\mathrm{new}},q]\) is collision-free, if
   \[
   c(q_{\mathrm{new}}) + c(q_{\mathrm{new}}\to q) < c(q),
   \]
   then **change \(q\)’s parent to \(q_{\mathrm{new}}\)** and update \(c(q)\) (and, if maintained, the costs of its descendants).

7. **Goal check.** If \(q_{\mathrm{new}}\in\mathcal{C}_{\mathrm{goal}}\), record/update the current best path.
...
::: {.examplebox}
::: {.example #rrt-star-2d name="RRT\* for 2D Kinematic Motion Planning"}

We can apply RRT\* to find collision-free paths from a start position to a goal region.

You can play with the code [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/rrt_star_2d.py).

The code provides an implementation of RRT\* from scratch. For an off-the-shelf implementation of RRT\* and other motion planning algorithms, we recommend the [Open Motion Planning Library (OMPL)](https://ompl.kavrakilab.org).

```{r rrt-star-2d-kinematic, out.width='80%', fig.show='hold', fig.cap='RRT* finding collision-free paths.', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Model-based-optimization/rrt_star_kinematic_1.gif')
knitr::include_graphics('images/Model-based-optimization/rrt_star_kinematic_2.gif')
```
:::
:::
...
\]
Given \(x_\mathrm{start}\in\mathcal{X}_\mathrm{free}\) and a goal region \(\mathcal{X}_\mathrm{goal}\subset\mathcal{X}_\mathrm{free}\), we seek a *feasible trajectory*
\[
...
::: {.examplebox}
::: {.example #dubins-distance name="Distance Metric for Dubins Car"}

In kinodynamic planning, two states that are close in Euclidean norm can still be far in terms of *actual time/effort to move between them under the robot's dynamics*. A system-aware local distance replaces a geometric norm
\[
\|x_i-x_j\|
\]
...
**Forward "steering" (local planner).** When no closed-form two-point connector exists, *forward propagate* from \(x_{\mathrm{near}}\) by sampling controls and durations and integrating:
\[
...
1. **Goal-biased sampling.** Draw \(x_{\mathrm{rand}}\) from \(\mathcal{X}_\mathrm{goal}\) with probability \(p_{\mathrm{goal}}\), else from \(\mathcal{X}_\mathrm{free}\).

2. **Nearest.** Select \(x_{\mathrm{near}}\in\arg\min_{x\in V(T)} d(x,\,x_{\mathrm{rand}})\).

3. **Forward "steer".** Generate a batch \(\{(u_k,\tau_k)\}_{k=1}^{K}\), integrate \(\dot{x}=f(x,u_k)\) from \(x_{\mathrm{near}}\) up to \(\tau_k\), discard infeasible rollouts, and pick a candidate terminal \(x_{\mathrm{new}}\) (e.g., closest to \(x_{\mathrm{rand}}\) or lowest edge cost). If none feasible, **continue**.
...
5. **Choose parent by cost (feasible-only).** Provisional parent \(\bar{x}\gets x_{\mathrm{near}}\) with provisional cost
   \[
...
6. **Insert.** Add \(x_{\mathrm{new}}\) to \(V(T)\); add edge \((\bar{x}\to x_{\mathrm{new}})\) (storing its control/duration); set \(c(x_{\mathrm{new}})=\bar{c}\).

7. **Rewire (forward-only).** For each \(x\in\mathcal{N}_n(x_{\mathrm{new}})\), attempt a feasible connection \(x_{\mathrm{new}}\to x\). If
   \[
   c(x_{\mathrm{new}}) + c\big(x_{\mathrm{new}}\!\to\!x\big) < c(x),
   \]
   then **change \(x\)’s parent to \(x_{\mathrm{new}}\)** and update costs (and, if maintained, descendants).

8. **Goal check.** If \(x_{\mathrm{new}}\in\mathcal{X}_\mathrm{goal}\), record/update the current best trajectory.
...
::: {.examplebox}
::: {.example #rrtstar-dubins name="RRT* for Dubins Car"}
...
You can play with the code [here](https://github.com/ComputationalRobotics/2025-ES-AM-158-LECTURE-CODE/blob/main/rrt_star_dubins_v2.py).

A similar YouTube demonstration video can be found [here](https://youtu.be/ZH6NHrALBAg?si=F5nCbuO_d_qsLcJx).

```{r rrtstar-dubins, out.width='70%', fig.show='hold', fig.cap='RRT* for Dubins Car.', fig.align='center', echo=FALSE}
knitr::include_graphics('images/Model-based-optimization/rrtstar_dubins.gif')
```
