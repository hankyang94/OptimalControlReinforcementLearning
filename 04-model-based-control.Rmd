...
::: {.examplebox}
::: {.example #sqp-to name="Trajectory Optimization with SQP"}
...
<!-- For later use (e.g., analytic Jacobians in SQP), the **discrete-time Jacobians** are
\[
A_k := \frac{\partial f_h}{\partial x}(x_k,u_k)
=
\begin{bmatrix}
1 & 0 & -h v_k \sin\theta_k\\
0 & 1 & \ \ h v_k \cos\theta_k\\
0 & 0 & 1
\end{bmatrix},
\qquad
B_k := \frac{\partial f_h}{\partial u}(x_k,u_k)
=
\begin{bmatrix}
h\cos\theta_k & 0\\
h\sin\theta_k & 0\\
...
(\#eq:uni-obj)
\]
where the outer factor \(\tfrac12\) is conventional in quadratic objectives.
...
**Two obstacles (failure).** However, when we add a third obstacle, Fig. \@ref(fig:unicycle-to-sqp-3obs) shows that the SQP algorithm converges to an infeasible solution that collides with the obstacles.
